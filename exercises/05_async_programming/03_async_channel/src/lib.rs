...
pub async fn producer_consumer(items: Vec<String>) -> Vec<String> {
    ...
    let (send,mut receive)=mpsc::channel(items.len().max(1));
    let producer=tokio::spawn(async move {
        for item in items {
            if let Err(_)=send.send(item).await {
                break;
            }
        }
    });
    ...
    let final_results = consumer.await.unwrap();
...
pub async fn fan_in(n_producers: usize) -> Vec<String> {
    ...
    let (send,mut receive)=mpsc::channel(n_producers.max(1));
    for i in 0..n_producers {
        let sd=send.clone();
        tokio::spawn(async move {
            sd.send(format!("producer {i}: message")).await.unwrap();
        }).await.unwrap();
    }
    drop(send);
    let mut results=vec![];
    tokio::spawn(async move {
        while let Some(m)=receive.recv().await {
            results.push(m);
        }
        results
    }).await.unwrap()
