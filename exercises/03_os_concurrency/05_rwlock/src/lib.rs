...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        loop {
            ...
            if bits & WRITER_HOLDING!=0 || bits&WRITER_WAITING!=0 || bits&READER_MASK==READER_MASK {
                core::hint::spin_loop();
                continue;
            }else{
                match self.state.compare_exchange(bits, bits+1, Ordering::AcqRel, Ordering::Acquire) {
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop {
            let bits=self.state.load(Ordering::Acquire);
            if bits&WRITER_HOLDING !=0 || bits&READER_MASK!=0 {
                core::hint::spin_loop();
                continue;
            }
            match self.state.compare_exchange(bits, WRITER_HOLDING, Ordering::AcqRel, Ordering::Acquire) {
